import React, { useCallback, useState, lazy, memo } from 'react';
import { Heart as HeartIcon, Check as CheckIcon } from 'lucide-react';
import { motion } from 'framer-motion';
import { CartItemAnimation } from './animations/CartItemAnimation';
  ...
  return <>
      ...
      contain: 'content'
          ...
          <div className="relative aspect-[4/3] overflow-hidden rounded-t-xl">
            ...
            <img srcSet={`${meal.image}?w=400&q=75 1x, ${meal.image}?w=800&q=75 2x`} src={`${meal.image}?w=400&q=75`} alt={meal.name} loading="lazy" width={400} height={300} onLoad={() => setImageLoaded(true)} className={`w-full h-full object-cover transition-opacity duration-300 ${imageLoaded ? 'opacity-100' : 'opacity-0'}`} style={{
        ...
        {showAnimation && animationConfig && <CartItemAnimation imageUrl={meal.image} {...animationConfig} onComplete={() => setShowAnimation(false)} />}
      </>;
});
